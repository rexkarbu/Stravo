...
- [x] **Komponen Speedometer Besar Neon (`lib/core/widgets/neon_speedometer_gauge.dart`)**
  - [x] Busur speedometer melingkar dengan indikator jarum / progress glow
  - [x] Angka kecepatan ukuran ekstra besar (64px) di tengah
- [x] **Badge Indikator Permukaan Jalan (`lib/core/widgets/surface_type_badge.dart`)**
  - [x] Tag kecil penanda: *Aspal Mulus*, *Gravel Halus*, *Makadam Kasar*, *Singletrack*
- [x] **Indikator Kategori Tanjakan (`lib/core/widgets/climb_gradient_indicator.dart`)**
  - [x] Tag persen kecuraman dengan warna dinamis: Hijau (<4%), Kuning (5-8%), Merah (>13%)
- [x] **Badge Selisih Waktu Ghost Pacer (`lib/core/widgets/ghost_pacer_badge.dart`)**
  - [x] Hijau jika unggul (`+2.4s ahead`), Merah jika tertinggal (`-1.8s behind`)
- [x] **Layar Galeri Komponen (`lib/features/debug/design_system_catalog_screen.dart`)**
  - [x] Satu layar tempat Ray bisa melihat semua tombol, kartu, dan indikator yang telah dibuat
